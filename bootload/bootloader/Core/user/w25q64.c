...
void flash_sector_erase(uint32_t sector_addr)
{
    ...
    W25Q64_CS(0);
    flash_send_byte(0x20);
    flash_send_byte((sector_addr & 0xFF0000) >> 16);
    flash_send_byte((sector_addr & 0xFF00) >> 8);
    flash_send_byte(sector_addr & 0xFF);
...
/**
  ...
  * @retval None
  */
void flash_block_erase(void)
{
    flash_write_enable();

    W25Q64_CS(0);
    flash_send_byte(0xC7);
    W25Q64_CS(1);
